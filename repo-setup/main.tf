...
data "external" "project_check" {
  program = ["sh", "-c", "PROJECT_PATH=$(echo '${var.gitlab_group}/${var.app_name}' | sed 's/\\//%2F/g'); RESPONSE=$(wget -q -O - --header='PRIVATE-TOKEN: ${var.gitlab_token}' https://gitlab.com/api/v4/projects/$PROJECT_PATH 2>/dev/null || echo ''); PROJECT_ID=$(echo \"$RESPONSE\" | grep -o '\"id\":[0-9]*' | head -1 | grep -o '[0-9]*'); if [ -n \"$PROJECT_ID\" ]; then echo '{\"exists\": \"true\", \"id\": \"'$PROJECT_ID'\"}'; else echo '{\"exists\": \"false\", \"id\": \"0\"}'; fi"]
}
